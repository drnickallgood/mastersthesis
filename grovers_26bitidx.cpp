...
int main()
{
    ...
    const bitLenInt valueLength = 10;
    ...
    // how many total qubits do i want to simulate
    bitLenInt qubits = 20;
    // Both CPU and GPU types share the QInterface API.
#if ENABLE_OPENCL
    //QInterfacePtr qReg = CreateQuantumInterface(QINTERFACE_OPENCL, 20, 0);
    QInterfacePtr qReg = CreateQuantumInterface(QINTERFACE_OPENCL, qubits, 0);
#else
    //QInterfacePtr qReg = CreateQuantumInterface(QINTERFACE_CPU, 20, 0);
    QInterfacePtr qReg = CreateQuantumInterface(QINTERFACE_CPU, qubits, 0);
#endif
    ...
    std::cout << "Chance of match:" << qReg->ProbAll(TARGET_VALUE | (TARGET_KEY << valueLength)) << std::endl << "Current match: " << qReg->MReg(0,qubits) << std::endl;
